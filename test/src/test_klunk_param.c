...
void klunk_param_test()
{
	...
	param = fastcgi_parameter_create(value, value_len, name, name_len);
	TEST_ASSERT_NOT_EQUAL(param, 0);
	if (param != 0) {
		str_result = strlen(param->name);
		TEST_ASSERT_EQUAL(str_result, (int)value_len);
...
void klunk_param_llist_test()
{
	...
	value = malloc(256);
	assert(value != 0);
	list = llist_create(sizeof(fastcgi_parameter_t));
	assert(list != 0);
	result = llist_register_dtor(list, fastcgi_parameter_destroy);
	...
	for (n = 0; n < 10; n++) {
		...
		param = fastcgi_parameter_create(name, name_len, value, value_len);
		assert(param != 0);
		llist_add(list, param, sizeof(fastcgi_parameter_t));
	}
	...
	for (n = 0; n < 10; n++) {
		name_len = snprintf(name, 256, "item %d", n);
		value_len = snprintf(value, 256, "value %d", n);
		param = (fastcgi_parameter_t*)ll_item->data;
