...
void fcgi_param_test()
{
	...
	param = fcgi_param_create(name, name_len, value, value_len);
	...
	if (param != 0) {
		...
		fcgi_param_destroy(param);
	}
	param = fcgi_param_create(value, value_len, name, name_len);
	TEST_ASSERT_NOT_EQUAL(param, 0);
	if (param != 0) {
		str_result = strlen(param->name);
		TEST_ASSERT_EQUAL(str_result, (int)value_len);
...
void fcgi_param_llist_test()
{
	...
	value = malloc(256);
	assert(value != 0);
	list = llist_create(sizeof(fcgi_param_t));
	assert(list != 0);
	result = llist_register_dtor(list, (llist_item_dtor)&fcgi_param_destroy);
	...
	for (n = 0; n < 10; n++) {
		...
		param = fcgi_param_create(name, name_len, value, value_len);
		assert(param != 0);
		llist_add(list, param, sizeof(fcgi_param_t));
	}
	...
	for (n = 0; n < 10; n++) {
		name_len = snprintf(name, 256, "item %d", n);
		value_len = snprintf(value, 256, "value %d", n);
		param = (fcgi_param_t*)ll_item->data;
